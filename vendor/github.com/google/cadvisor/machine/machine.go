...
var (
	...
	//cpuClockSpeedMHz     = regexp.MustCompile(`(?:cpu MHz|clock)\s*:\s*([0-9]+\.[0-9]+)(?:MHz)?`) 
        cpuClockSpeedMHz  = regexp.MustCompile(`(?:CPU frequency \[MHz\])\s*:\s*([0-9]+\.[0-9]+)(?:\[MHz\])?`)
        swcpuClockSpeedMHz  = regexp.MustCompile(`(?:CPU frequency \[MHz\])\s*:\s*([0-9]+\.[0-9]+)(?:\[MHz\])?`)
...
func GetClockSpeed(procInfo []byte) (uint64, error) {
	...
	if utils.FileExists(maxFreqFile) {
        ...
        var matches [][]byte
        if isSw64() {
                matches = swcpuClockSpeedMHz.FindSubmatch(procInfo)
                } else {
